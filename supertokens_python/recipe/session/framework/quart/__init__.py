...
def verify_session(
    ...
    def session_verify(f: _T) -> _T:
        @wraps(f)
        async def wrapped_function(*args: Any, **kwargs: Any):
            nonlocal user_context
            from quart import make_response, request

            base_req = QuartRequest(request)
            ...
            try:
                ...
                return (
                    await make_response(response) if response is not None else None
                )
            except SuperTokensError as e:
                response = QuartResponse(await make_response("", 200))
